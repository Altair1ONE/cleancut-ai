...
export function generateMetadata({
  ...
  if (!post) {
    return { title: "Blog", description: "CleanCut AI blog" };
  }
...
export default function BlogPostPage({ params }: { params: { slug: string } }) {
    ...
    return (
      <main className="mx-auto max-w-3xl px-4 py-10">
        ...
        <div className="mt-4 rounded-2xl border border-slate-800 bg-slate-900/40 p-4 text-sm text-slate-200">
          ...
          <div className="mt-2">
            params.slug: <span className="text-rose-300">{params.slug}</span>
          </div>
          <div className="mt-2">Available slugs:</div>
        ...
        <Link href="/blog" className="mt-6 inline-block text-indigo-300 hover:underline">
          Back to blog
        </Link>
      </main>
    );
  ...
  return (
    <main className="mx-auto max-w-3xl px-4 py-10">
      ...
      <h1 className="mt-2 text-3xl font-bold text-white md:text-4xl">{post.title}</h1>
